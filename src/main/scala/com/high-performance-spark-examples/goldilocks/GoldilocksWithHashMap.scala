...
object GoldilocksWithHashMap {
  ...
  def findRankStatistics(dataFrame: DataFrame, targetRanks: List[Long]):
    ...
    // 每一列中的((8.0,4),1)每个元素都只出现一次
    val aggregatedValueColumnPairs: RDD[((Double, Int), Long)] = getAggregatedValueColumnPairs(dataFrame)
    println("#########   aggregatedValueColumnPairs   #############")
    aggregatedValueColumnPairs.collect().foreach(println)
   ...
   *    ((1.5, 0), 2) ((1.25, 1), 1) ((2.5, 1), 1) ((2.0, 2), 2) 【第二列值为2.0的个数】
   *
   ...
   */
  //tag::hashMap_step2[]
  private def getColumnsFreqPerPartition(sortedAggregatedValueColumnPairs: RDD[((Double, Int), Long)], numOfColumns : Int): Array[(Int, Array[Long])] = {
    val zero = Array.fill[Long](numOfColumns)(0)
    println("######## getColumnsFreqPerPartition ########")
    println()
    // 使用aggregate实现的
    def aggregateColumnFrequencies(partitionIndex : Int, pairs : Iterator[((Double, Int), Long)]) = {
      ...
      val columnsFreq : Array[Long] = pairs.aggregate(zero)(
        (a : Array[Long], v : ((Double,Int), Long)) => { // a是得到的结果，v是pairs遍历的元素
